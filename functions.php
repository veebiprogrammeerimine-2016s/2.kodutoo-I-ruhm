<?php
	...
	function signup($email, $password, $username, $name, $d_o_b, $now) {
		...
		$check_username_uniqueness_stmt = $mysqli->prepare("SELECT username FROM users WHERE username = ?");
		...
		$check_email_uniqueness_stmt = $mysqli->prepare("SELECT email FROM users WHERE email = ?");		
