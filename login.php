...
<?php
	...
	if (isset($_POST['day_of_birth']) &&
		...
		empty($date_of_birth_Error)) {
			...
			signup($signup_email, $signup_password_hash, $signup_username, $signup_name, $date_of_birth, $now);
			$signup_name = '';
			$signup_username = '';
			$signup_email = '';
			$date_of_birth_Error = 'tere';
	}
	/*
	if ($date_of_birth_Error != '') {
		$day_of_birth = 'day01';
		$month_of_birth = 'mont01';
		$year_of_birth = 'year2016';
		if ($date_of_birth_Error == 'tere') {
			$date_of_birth_Error = '';
		}
	}
	*/
...
<html>
	...
	<!-- atribuudid asuvad algus tagi sees -->
		...
		<form method = "POST">
			...
			<table>
				<tr>
					<td> <input name = "login_username" type = "text" placeholder = "Kasutajanimi" value = '<?=$login_username?>'> </td> <td style = 'color:red'> <?=$login_username_Error?> </td>
				</tr>
				<tr>
					<td> <input name = "login_password" type = "password" placeholder = "Parool"> </td> <td style = 'color:red'> <?=$login_password_Error?> </td>
				</tr>
				<tr>
					<td style = 'text-align:right'> <input type = "submit" value = "Logi sisse"> </td>
				</tr>
			</table>
